...
exports.DashboardPage = class DashboardPage {
  constructor(page) {
    ...
    this.additionsCaptureSelectBtn = this.page.locator(
      "//span[normalize-space()='Additions Captured']/following-sibling::span[normalize-space()='Select']"
    );
    this.applyBtn = this.page.locator("//button[normalize-space()='Apply']");
    this.checkLineIcon = this.page.locator("//icon[@name='check_line']");
    this.additionsCapturedWidget = this.isMobile
      ? this.page.locator(
          "(//app-additions-captured)[2]//div[normalize-space()='Additions Captured']"
        )
      : this.page.locator(
          "(//app-additions-captured)[1]//div[normalize-space()='Additions Captured']"
        );
    this.myWidgetTxt = this.page.locator("//span[normalize-space()='My Widgets']");
    this.dateSelect = this.isMobile
      ? this.page.locator("(//select[@formcontrolname='datePeriod'])[2]")
      : this.page.locator("(//select[@formcontrolname='datePeriod'])[1]");
    this.startDateRangeInput = this.isMobile
      ? this.page.locator("(//input[@formcontrolname='startDate'])[2]//following-sibling::span")
    ...
    this.textOfEquipment = this.page.locator(
      "//span[contains(@class,'e2e_add_on_request_product_name')]"
    );
    this.nextButton = this.page.locator("//span[text()='Next']");
    this.selectFirstDate = this.page.locator(
      "//form//ul/div/li[1]//span[contains(text(),'select')]"
    );
    this.selectSecondDate = this.page.locator(
      "//form//ul/div/li[2]//span[contains(text(),'select')]"
    );
    ...
    this.productInWidget = equipment =>
      this.isMobile
        ? this.page.locator(`(//div[text()='${equipment}'])[2]`)
  ...
  async addingWidgets() {
    ...
    try {
      await assertElementNotVisible(
        this.additionsCapturedWidget,
        'Verify the default state of the Dashboard page'
      );
    } catch {
    ...
    await assertElementVisible(
      this.additionsCaptureSelectBtn,
      'Verify widget selection modal should be displayed'
    );
    await executeStep(
      this.additionsCaptureSelectBtn,
      'click',
      "Select 'Additions Captured' widget "
    );
    await executeStep(this.applyBtn, 'click', "click 'Apply' button");
    await this.page.waitForTimeout(parseInt(process.env.medium_timeout));
    await assertElementVisible(
      this.additionsCapturedWidget,
      "Verify 'Additions Captured' widget should be added to the Dashboard"
    );
    ...
    await assertElementVisible(
      this.additionsCapturedWidget,
      "Verify 'Additions Captured' widget is still displayed on the Dashboard"
    );
  }
  ...
  async removeWidgets() {
    await assertElementVisible(
      this.additionsCapturedWidget,
      "Verify  'Additions Captured' widget is displayed properly"
    );
    ...
    await executeStep(
      this.checkLineIcon,
      'click',
      "Click on the green checkmark to de-select 'Additions"
    );
    await assertElementVisible(
      this.additionsCaptureSelectBtn,
      "Verify the 'Additions Captured' widget should be de-selected"
    );
    await executeStep(this.applyBtn, 'click', "Click on 'Apply' button");
    await this.page.waitForTimeout(parseInt(process.env.small_timeout));
    await assertElementNotVisible(
      this.additionsCaptureSelectBtn,
      "Verify the 'Additions Captured' widget should be removed successfully from the Dashboard page."
    );
    await test.step('Refresh the page', async () => {
      await this.page.reload();
    });
    await assertElementNotVisible(
      this.additionsCaptureSelectBtn,
      "Verify the 'Additions Captured' widget should NOT be displayed on the Dashboard page."
    );
  }
  ...
  async assertWidget() {
    if (await this.additionsCapturedWidget.isVisible()) {
      await assertElementVisible(
        this.additionsCapturedWidget,
        'Verify that addition capture widget is visible'
      );
    } else {
  ...
  async assertElementsInWidgets() {
    ...
    await assertEqualValues(
      selectedOption,
      indexPage.lighthouse_data.currentWeek,
      'Verify that the current week should be selected by default.'
    );
    await assertElementContainsText(
      this.startDateRangeInput,
      getWeekStartDate(),
      'Verify exact dates should be displayed properly.'
    );
    await this.page.waitForTimeout(parseInt(process.env.small_timeout));
    await test.step('Verify that valid results should be returned for each time slot.', async () => {
      ...
      await assertElementContainsText(
        this.startDateRangeInput,
        getLastWeekStartDate(),
        'Verify exact dates should be displayed properly for the last week.'
      );
      await this.page.waitForTimeout(parseInt(process.env.small_timeout));
      ...
      await assertElementContainsText(
        this.startDateRangeInput,
        getLastMonthStartDate(),
        'Verify exact dates should be displayed properly for the last month.'
      );
    });
  ...
  async createAddOnForWidget(requestedBy, individualProduct, quantity, isDiscuntNecessary) {
    ...
    if (isDiscuntNecessary) {
      await executeStep(this.discountInput, 'fill', 'Enter discount percentage', [
        indexPage.lighthouse_data.discountPercentage
      ]);
    }
  }
  ...
  async assertAddOnWith1pcsFor1DayWithoutDiscount() {
    await this.createAddOnForWidget(
    ...
    await assertElementVisible(
      this.productInWidget(getText),
      'Verify that Item A should be displayed properly for Add on with 1pcs for 1 days and without discount.'
    );
  }
  ...
  async assertAddOnWith2PcsFor2DaysWithoutDiscount() {
    await this.createAddOnForWidget(
      indexPage.lighthouse_data.requestedBy,
      indexPage.lighthouse_data.individualProduct,
      indexPage.lighthouse_data.quantity2,
      false
    );
    const getText = await this.getTextOfTheEquipment();
    ...
    await assertElementVisible(
      this.productInWidget(getText),
      'Verify that Item should be displayed properly for Add on with 2pcs for 2 days and without discount.'
    );
  }
  ...
  async assertAddonWith1pcsFor1DayWith25PerDiscount() {
    ...
    await assertElementVisible(
      this.productInWidget(getText),
      'Verify that Item A should be displayed properly for Add on with 1pcs for 1 days and with 25% discount.'
    );
  }
};
