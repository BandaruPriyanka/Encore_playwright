...
exports.FlowSheetPage = class FlowSheetPage {
  constructor(page) {
    ...
    this.locationDiv = this.page.locator(
      "//app-header//app-notifications/../div"
    );
    this.searchLocation = this.page.locator(
      "//input[@placeholder='Search location']"
    );
    this.selectLocation = this.page.locator(
      "//span[contains(text(),'1137-Hotel Del Coronado')]"
    );
    this.roomsCount = this.page.locator(
      "//div[text()=' Rooms ']/following-sibling::div"
    );
    this.crossButton = this.page.locator("//icon[@name='cross_line']");
    this.flowsheetDiv = this.page.locator("//div[@class='flex']/child::div[1]");
    this.jobIdChecking = (jobId) =>
      this.page.locator(`//span[contains(text(), '#${jobId}')]`);
    ...
    this.customerName = this.page.locator(
      "//div[normalize-space()='Customer Name']"
    );
    this.actionTab = this.page.locator(
      "(//div[normalize-space()='Action'])[1]"
    );
    this.strikeReset = this.page.locator(
      "//div[normalize-space()='Strike Reset']"
    );
    this.applyFilter = this.page.locator("//span[text()=' Apply filters ']");
    this.filterCount = this.page.locator(
      "//icon[@name='filter_bulk']/parent::div/div"
    );
    this.clearFilter = this.page.locator(
      "//span[normalize-space()='Clear filters']"
    );
  ...
  async changeLocation(locationId) {
    ...
    await executeStep(
      this.searchLocation,
      "fill",
      "Fill the search location field",
      [locationId]
    );
  ...
  async checkingSearchFunctionality() {
    ...
    await this.searchFunction(
      indexPage.navigator_data.order_name.toUpperCase().trim()
    );
  ...
  async clickonPreviousWeek() {
    await executeStep(
      this.previousweekIcon,
      "click",
      "click on previous week icon",
      []
    );
  }
